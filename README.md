...
- In `Eff()`, all nuisance regressions are **continuous-valued** (even
  when `y` is binary), so `"logistic KRR"` / `"glmnet_logistic"` are
  automatically mapped to `"KRR"` / `"glmnet_linear"` with a warning.
...
> used when you set `nonpara_method = "mixed KRR"` (or `"logistic KRR"`
> for binary regressions). If you set `nonpara_method = "KRR"` (plain
> RBF on all columns) or `"glmnet_*"` (elastic-net), dfSEDI will not use
> the delta kernel.
