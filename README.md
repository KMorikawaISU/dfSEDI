...
# Load the bundled example script (defines generate_dualframe_population and run_mc)
...
in real data), you can supply `X` as a `data.frame` and mark discrete
variables as `factor`. For example, the bundled DGP has a binary
covariate in `Scenario = 3`:

``` r
dat3 <- generate_dualframe_population(N = N, Scenario = 3)
...
## Example: Monte Carlo simulation helper (`run_mc`)

The bundled example script `inst/examples/dualframe_simulation.R` also
defines a simple Monte Carlo runner `run_mc()` that repeatedly generates
datasets and computes:

- `P`, `NP`, `NP_P`
- `Eff` (full data; `x_info = TRUE`)
- `Eff_union_dat` (union sample only; `x_info = FALSE`)
- `Eff_S`, `Eff_P`

`run_mc()` returns a **long-format** data frame containing point
estimates (`theta`), sandwich SEs (`se`), and 95% CIs (`ci_l`, `ci_u`)
for each estimator in each replication, along with realized sample
sizes.
...
# Scenario = 1 corresponds to a paper-style S1 setup in the bundled DGP
res <- run_mc(
  B             = B,
  N             = N,
  Scenario      = 1,
...
# Optional summary (true theta is 0 in the bundled scenarios)
summarize_mc(res, theta_true = 0)
```

To run the other bundled scenarios, set `Scenario = 2` or
`Scenario = 3`.
...
These are mainly intended for **simulation settings** where `dat`
contains a “population-like” structure (including \\(0,0)\\ units) and
`pi_p` is defined for the full frame.
...
fit_effP_lin <- Eff_P(dat, K = 2, logit = FALSE)
```
