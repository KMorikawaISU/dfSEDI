...
### Important: handling the \\(d\_{np}, d_p) = (0,0)\\ pattern (`x_info`)

In the paper’s notation, there are four response patterns
\\(\delta\_{NP}, \delta_P)\in\\0,1\\^2\\. In many *practical*
...
the union sample \\\delta\_{NP}\cup\delta_P = 1\\. In that common case,
there are **no rows** with \\(d\_{np}, d_p)=(0,0)\\.

The paper also notes that the theory covers the case where \\X\\ is
unavailable when \\(\delta\_{NP},\delta_P)=(0,0)\\; in that case, it
suffices to set the augmentation functions \\h_4^*(X)\\ and
\\\eta_4^*(X)\\ to zero.
...
- `x_info = FALSE` (**recommended for most real-data use cases**):  
  ...
  union sample, or you do not have covariates \\X\\ for \\(0,0)\\ units.
  It also avoids computations involving the \\(0,0)\\ pattern and can be
...
dfSEDI estimates nuisance functions (e.g., \\\mu(X)=E\[Y\mid X\]\\,
\\\pi_P\\, \\\eta_4^*(X)\\, \\h_4^*(X)\\) using kernel methods.
...
- `base_fun` must be a function of the form
  `base_fun <- function(X) { ... }`
- `X` is the \\n \times p\\ design matrix extracted from `dat` (via
  `df_get_X(dat)`)
...
## Monte Carlo simulation (serial / parallel with progress)
...
× estimator), so it is easy to summarize or plot.
...
res <- run_mc(
  ...
  K = 2,
  seed_start = 1,
  show_progress = TRUE,        # text progress bar
  progress_each_fit = FALSE
)

summary <- summarize_mc(res, theta_true = 0)
summary
```

### Parallel MC + progress bar (recommended cross-platform)

Base `parallel` does not provide an easy progress bar for parallel
workers. For a progress bar **with parallel execution**, we recommend
the *snow* cluster approach with `pbapply`.

**Needed packages (Suggests):** `parallel`, `pbapply`
...
# (Optional) show a nice text progress bar
pbapply::pboptions(type = "txt")

# Ensure workers can call dfSEDI + the example functions
...
B <- 100
seed_start <- 1
seeds <- seed_start + seq_len(B) - 1

# One replication -> one long-format data.frame
one_rep_long <- function(seed) {
...
# Progress bar appears in the master process while workers run in parallel
out_list <- pbapply::pblapply(seeds, function(s) {
  set.seed(s)
  one_rep_long(s)
}, cl = cl)
...
summary_par <- summarize_mc(res_par, theta_true = 0)
summary_par
```

### Notes

- **Windows users:** use the snow cluster approach above (works
  everywhere).
- **macOS/Linux users:** you can also use fork-based parallelism
  (`mclapply`), but progress bars are trickier. If you want a progress
  bar in multicore mode, consider `pbmcapply` (Suggests) instead.

If you want to list optional packages in DESCRIPTION:

- `pbapply` (progress bar for snow parallel)
- `pbmcapply` (progress bar for multicore; macOS/Linux)
...
> Tip: If you do not have \\(0,0)\\ units / do not have \\X\\ for them,
> using `x_info = FALSE` can be much faster. This is common when your
> dataset is the union sample only.

------------------------------------------------------------------------

## Comparison estimators (P / NP / NP ∪ P) ✅ Updated: population size `N` and Hajek option

For simulations, it is sometimes useful to compare `Eff` against simpler
estimators:

- `df_estimate_P()` : probability-only estimator (HT-type)
- `df_estimate_NP()` : non-probability-only estimator (Chang &
  Kott-type; requires `base_fun`)
- `df_estimate_NP_P()` : NP union P estimator (Chang & Kott-type;
  requires `base_fun`)

### Why `N` matters (union-only input)

`df_estimate_P()` / `df_estimate_NP()` / `df_estimate_NP_P()` compute
\\\theta=E(Y)\\ using contributions that are naturally on the
**population-total** scale. When your `dat` includes the **full frame**
(i.e., it contains \\(d\_{np}, d_p)=(0,0)\\ rows), the functions can
safely use `N = nrow(dat)` internally.

However, if you only pass the **union sample** (no \\(0,0)\\ rows), then
`nrow(dat)` is **not** the population size and would create a scale
error. To prevent silent nonsense, dfSEDI **requires** you to specify
the population size `N` in that case **unless** you request the Hajek
(ratio) form.

### Function signatures (updated)

- `df_estimate_P(dat, N = NULL, hajek = FALSE)`
- `df_estimate_NP(dat, base_fun, phi_start = NULL, max_iter = 20, N = NULL, hajek = FALSE)`
- `df_estimate_NP_P(dat, base_fun, phi_start = NULL, max_iter = 20, N = NULL, hajek = FALSE)`

**Defaults are output-preserving**: if `dat` contains full-frame rows,
omitting `N` and `hajek` reproduces the previous behavior and output
fields.

### Examples

**(A) Full-frame `dat` (simulation-like): no need to pass `N`**
...
**(B) Union-only `dat_union`: pass `N` (HT-style mean over the full
population)**

``` r
dat_union <- subset(dat, d_np == 1 | d_p == 1)

N_pop <- 10000  # known population/frame size

fit_P     <- df_estimate_P(dat_union, N = N_pop)
fit_NP    <- df_estimate_NP(dat_union, base_fun = base_fun, N = N_pop)
fit_NP_P  <- df_estimate_NP_P(dat_union, base_fun = base_fun, N = N_pop)
```

**(C) Union-only `dat_union`: Hajek (ratio) form (does not require
`N`)**

``` r
fit_P_hajek    <- df_estimate_P(dat_union, hajek = TRUE)
fit_NP_hajek   <- df_estimate_NP(dat_union, base_fun = base_fun, hajek = TRUE)
fit_NP_P_hajek <- df_estimate_NP_P(dat_union, base_fun = base_fun, hajek = TRUE)

# Hajek results are returned as additional fields:
fit_P_hajek$hajek_theta
fit_P_hajek$hajek_se
fit_P_hajek$hajek_ci
```

Each estimator always returns `theta`, `se`, `ci`:
...
If `hajek = TRUE`, the returned object also includes `hajek_theta`,
`hajek_se`, and `hajek_ci` (while preserving the original `theta/se/ci`
fields).

> Note: `df_estimate_NP()` / `df_estimate_NP_P()` solve estimating
> equations and may return `NA` if the solver fails to converge. This is
> expected in some small samples or unstable designs.
...
involves the outcome regression \\\mu(X)=E(Y \mid X)\\.
...
### Eff_P with union sample only (`x_info = FALSE`)

If your input data contain only sampled units (union sample), set
`x_info = FALSE`:

``` r
dat_union <- subset(dat, d_np == 1 | d_p == 1)

fit_effP_union <- Eff_P(
  dat      = dat_union,
  x_info   = FALSE,
  progress = TRUE
)
...
If the outcome `y` is binary (0/1), dfSEDI can use logistic models for
the outcome regression \\\mu(X)=P(Y=1\mid X)\\:

- **Eff_S**: \\\mu(X)\\ is estimated by **kernel logistic regression**
  (RBF / mixed kernel + ridge regularization).
- **Eff_P**: \\\mu(X)\\ is estimated by **parametric logistic
...
- `logical`,
- a 2-level `factor` (or `character`, which will be treated like a
  2-level factor).

By default, both `Eff_S()` and `Eff_P()` have `logit = NULL`, which
means:

- If `y` is binary, dfSEDI **automatically uses logistic models** for
  \\\mu(X)\\.
...
- If `y` is a 2-level factor, dfSEDI treats the **second level** as the
  success category (=1).
- If `y` is numeric with two unique values, dfSEDI treats the **larger
  value** as success (=1).

To avoid ambiguity, you can always recode explicitly to `0/1` (or
reorder factor levels) before calling dfSEDI.
...
If you call a comparison estimator (`df_estimate_P/NP/NP_P`) with
`hajek = TRUE`, the returned list also includes:

- `hajek_theta` : Hajek (ratio) estimate for \\\theta\\
- `hajek_se` : standard error for `hajek_theta` (delta-method)
- `hajek_ci` : 95% CI for `hajek_theta`
