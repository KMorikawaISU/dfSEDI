...
##
## Observation pattern (fixed; paper Table 1 style)
## - (d_np,d_p)=(0,0): y is missing (NA)
## - pi_p is observed only for d_p==1 (else NA)
## - True complete-data values are kept as y_true and pi_p_true.
##
## Parallel
...
## Prefer sourcing local dualframe_core.R (standalone use).
## If not present, fall back to the installed package.
if (file.exists("dualframe_core.R")) {
  source("dualframe_core.R")
}

if (!exists("Eff", mode = "function")) {
  if (!requireNamespace("dfSEDI", quietly = TRUE)) {
    stop("Please install dfSEDI or put dualframe_core.R in the working directory.")
  }
  suppressPackageStartupMessages(library(dfSEDI))
}
...
generate_dualframe_population <- function(N, Scenario = 1, pi_p_offset = 0.005) {
  ...
  if (sc %in% c(1, 2)) {
    lin_np <- -2.15 - 0.5 * x - 0.75 * y_true
  } else {
    lin_np <- -1.5 - 0.3 * cos(2 * y_true) - 0.1 * (y_true - 1)^2
  }
  pi_np <- plogis(lin_np)
  d_np  <- rbinom(N, 1, pi_np)

  lin_p <- -3.0 - 0.25 * (x - 2)^2
  pi_p_true <- pi_p_offset + plogis(lin_p)
  pi_p_true <- pmin(pmax(pi_p_true, 0), 1)
  d_p   <- rbinom(N, 1, pi_p_true)

  # Observed pi_p only for d_p==1 units
  pi_p_obs <- pi_p_true
  pi_p_obs[d_p == 0] <- NA_real_

  # Observed y only for union sample; (0,0) has y missing
  y_obs <- y_true
  y_obs[(d_np == 0) & (d_p == 0)] <- NA_real_
...
df_par_lapply_lb_progress <- function(cl, X, fun, show_progress = TRUE) {
  ...
  # Use internal functions explicitly; avoids "not exported" errors
