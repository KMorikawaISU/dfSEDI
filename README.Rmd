...
If you want `X` to contain an explicit categorical column (recommended in real data), you can supply `X` as a `data.frame`
and mark discrete variables as `factor`. For example, the bundled DGP has a binary covariate in `Scenario = 3`:
...
The bundled example script `inst/examples/dualframe_simulation.R` also defines a simple Monte Carlo runner `run_mc()`
that repeatedly generates datasets and computes:
...
`run_mc()` returns a **long-format** data frame containing point estimates (`theta`), sandwich SEs (`se`), and 95% CIs
(`ci_l`, `ci_u`) for each estimator in each replication, along with realized sample sizes.
...
To run the other bundled scenarios, set `Scenario = 2` or `Scenario = 3`.
