...
- `Eff`     : semiparametric efficient estimator (**default: DML2**, supports DML1 / DML2, K-fold)
...
- `y`      : outcome variable (continuous or binary).  
             In the paper-style observation pattern (Table 1), `y` can be missing (`NA`) for \((d_{np},d_p)=(0,0)\).
             dfSEDI does **not** use `y` from \((0,0)\) units for the main (leading) terms; the implementation avoids
...
- `pi_p`   : design inclusion probability for the probability sample.  
             In Table 1 style data, `pi_p` is observed only when `d_p==1` (else `NA`).
...
- Complete-data values are kept as `y_true` and `pi_p_true` in the simulated dataset
...
Windows does not support fork-based parallelism (`mclapply`). Use `parallel="snow"`.
...
Option B (portable + progress bar): use `parallel="snow"` (works on Windows/macOS/Linux).
...
- `parallel="snow"` uses a PSOCK cluster with load balancing and a progress bar in the master process.
- If you are running inside RStudio on macOS/Linux and see issues with forking, prefer `parallel="snow"`.
