...
- `x_info = FALSE` (**recommended default for most users**):  
  ...
  or you do not have covariates \(X\) for \((0,0)\) units. It also avoids any computations
  that would require summing over \((d_{np},d_p)=(0,0)\) and can be substantially faster.
...
> If you do not have \((0,0)\) units / do not have \(X\) for them, using `x_info = FALSE` can be much faster.
