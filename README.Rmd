...
- In `Eff()`, all nuisance regressions are **continuous-valued** (even when `y` is binary), so `"logistic KRR"` / `"glmnet_logistic"` are automatically mapped to `"KRR"` / `"glmnet_linear"` with a warning.
...
> Note: The mixed-type (RBF × delta) product-kernel behavior below is used when you set `nonpara_method = "mixed KRR"` (or `"logistic KRR"` for binary regressions).
