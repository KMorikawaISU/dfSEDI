...
so it is easy to summarize or plot.
...
Base `parallel` does not provide an easy progress bar for parallel workers.
For a progress bar **with parallel execution**, we recommend the *snow* cluster
approach with `pbapply`.
...
- **Windows users:** use the snow cluster approach above (works everywhere).
- **macOS/Linux users:** you can also use fork-based parallelism (`mclapply`), but
  progress bars are trickier. If you want a progress bar in multicore mode,
  consider `pbmcapply` (Suggests) instead.
...
`df_estimate_P()` / `df_estimate_NP()` / `df_estimate_NP_P()` compute \\(\\theta=E(Y)\\) using contributions that are naturally on the **population-total** scale.
When your `dat` includes the **full frame** (i.e., it contains \\((d_{np}, d_p)=(0,0)\\) rows), the functions can safely use `N = nrow(dat)` internally.

However, if you only pass the **union sample** (no \\((0,0)\\) rows), then `nrow(dat)` is **not** the population size and would create a scale error.
To prevent silent nonsense, dfSEDI **requires** you to specify the population size `N` in that case **unless** you request the Hajek (ratio) form.
...
**Defaults are output-preserving**: if `dat` contains full-frame rows, omitting `N` and `hajek` reproduces the previous behavior and output fields.
...
**(B) Union-only `dat_union`: pass `N` (HT-style mean over the full population)**
...
**(C) Union-only `dat_union`: Hajek (ratio) form (does not require `N`)**
...
If `hajek = TRUE`, the returned object also includes `hajek_theta`, `hajek_se`, and `hajek_ci`
(while preserving the original `theta/se/ci` fields).
...
If your input data contain only sampled units (union sample), set `x_info = FALSE`:
...
> Note on scaling: `Eff_P()` currently does **not** take a population-size argument `N`.
> If you need \(\theta=E(Y)\) on the full-frame mean scale when using a union-sample-only dataset,
> rescale the output by `n_union / N` (where `n_union = nrow(dat_union)` and `N` is the frame size):
...
If you call a comparison estimator (`df_estimate_P/NP/NP_P`) with `hajek = TRUE`,
