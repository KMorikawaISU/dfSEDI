...
while still using the **full** `X` for nuisance regressions such as \(\eta_4^*(X)\) and \(h_4^*(X)\).
...
When `x_info = TRUE`, `Eff()` estimates the optimal augmentation functions \(h_4^*(X)\) and \(\eta_4^*(X)\) via cross-fitted nonparametric regression (DML).
...
- `prob_only` only affects the estimation of \(h_4^*(X)\) and \(\eta_4^*(X)\). The target parameter and the main estimating equations are unchanged.
...
dfSEDI estimates several nuisance functions by regression (e.g., \(\mu(X)=E[Y\mid X]\), \(\bar\pi_P(L)\) imputation, and the DML nuisance terms \(h_4^*(X)\) / \(\eta_4^*(X)\)).
