...
If you plan to use mixed continuous/discrete covariates via `x_cont` / `x_disc` (Li & Racine-type regression), you also need the [`np`](https://cran.r-project.org/package=np) package:
...
- `X`      : covariates used in the parametric NP propensity model and some nuisance models  
             ...
             If neither `X` nor `x` is present, `df_get_X()` will construct `X` from
             `x_cont` / `x_disc` (see below) by coercing them to numeric.
- `y`      : outcome variable
...
Optionally, you can split the covariates used in the **nonparametric** nuisance models
into continuous and discrete parts:
...
If `x_disc` is present, dfSEDI uses Li & Racine (2007) mixed-type kernel regression
for the nuisance functions via the [`np`](https://cran.r-project.org/package=np) package.  
If `x_disc` is *not* present, dfSEDI falls back to Gaussian kernels via `kernlab::gausspr`
(continuous-only case), which matches older versions of the package.

Internally, `df_get_X(dat)` constructs the parametric design matrix as follows:
...
- otherwise, if `x_cont` and/or `x_disc` are present, they are combined into a numeric
  matrix (discrete covariates are coerced to numeric codes).

For nonparametric regression, `df_get_np_design(dat, include_y = FALSE/TRUE)` builds
a `data.frame` with continuous variables as numeric and discrete variables as factors,
...
DML2 conceptually estimates nuisances on the full data and computes a single estimate.
