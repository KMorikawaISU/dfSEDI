...
Option A (simplest and cross-platform): use the same PSOCK + `pbapply` approach as above (`parallel::makeCluster`, `pbapply::pblapply`).
...
- If you want to list optional packages in DESCRIPTION, add:
  ...
  - `pbmcapply` (progress bar for fork-based parallelism; macOS/Linux only)
