...
All estimators return a point estimate for theta = E(Y), and sandwich-type SE/CI computed from influence-function / pseudo-outcome contributions.
...
- X      : covariates used in the NP propensity and outcome models
...
Internally, dfSEDI extracts the covariate matrix via a helper:
- if dat$X exists, it is used (after coercion to numeric matrix)
...
Note: the current version focuses on inference for theta. Standard errors for phi are not provided (phi_se may be NULL).
